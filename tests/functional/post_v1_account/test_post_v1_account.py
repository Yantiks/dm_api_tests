...
def test_login_with_token():
    ...
    # активация токена
    activate_token = account_api.put_account_token(token=token)
    ...
    change_email = account_api.put_account_email(json_data=json_data_new)
    ...
    try_enter = login_api.post_account_login(json_data=json_data)
    print(try_enter.status_code)
    assert try_enter.status_code == 403, f"Пользователь не был авторизован, код ошибки: {try_enter.status_code}"
